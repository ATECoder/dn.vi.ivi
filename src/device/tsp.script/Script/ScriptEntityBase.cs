...
public abstract class ScriptEntityBase( FirmwareScriptBase firmwareScript, NodeEntityBase node ) : object()
{
    ...
    /// <returns>   True if the script needs to be embedded, false if not. </returns>
    public bool IsEmbedRequired()
    {
        return !this.Embedded || (this.FirmwareScript.EmbedToNonVolatileMemory && this.LoadedAsByteCode);
    }
