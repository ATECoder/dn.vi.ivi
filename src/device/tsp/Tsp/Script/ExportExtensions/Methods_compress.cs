...
public static partial class ExportExtensionsMethods
{
    ...
    /// <summary>   A string extension method that encrypts and compresses a script file into another. </summary>
    ...
    public static void CompressScriptFile( this string fromFilePath, string toFilePath, ScriptFormats fileFormat, IScriptCompressor compressor,
        IScriptEncryptor encryptor, bool overWrite = false, bool validate = true )
    {
        ...
        if ( validate && (fileFormat.HasFlag( ScriptFormats.Compressed ) || fileFormat.HasFlag( ScriptFormats.Encrypted )) )
        {
            ...
            if ( fromFilePathInfo.Length != tempFilePathInfo.Length )
                throw new InvalidOperationException( $"The script source file '{fromFilePath}' could not be validated; it's length {fromFilePathInfo.Length} is not the same as the length of the validation file {tempFilePathInfo.Length}." );
            if ( !File.ReadAllBytes( fromFilePath ).SequenceEqual( File.ReadAllBytes( tempFilePath ) ) )
                throw new InvalidOperationException( $"The script source file '{fromFilePath}' could not be validated; it is not equal to the validation file {tempFilePathInfo.Length}." );
        }
    }
        ...
        bool overWrite = false, bool validate = true )
    {
        ...
        if ( validate && (fileFormat.HasFlag( ScriptFormats.Compressed ) || fileFormat.HasFlag( ScriptFormats.Encrypted )) )
        {
            ...
            FileInfo tempFilePathInfo = new( tempFilePath );
            if ( fromFilePathInfo.Length != tempFilePathInfo.Length )
                throw new InvalidOperationException( $"The processed script source file '{fromFilePath}' could not be validated; it's length {fromFilePathInfo.Length} is not the same as the length of the validation file {tempFilePathInfo.Length}." );
            if ( !File.ReadAllBytes( fromFilePath ).SequenceEqual( File.ReadAllBytes( tempFilePath ) ) )
                throw new InvalidOperationException( $"The processed script source file '{fromFilePath}' could not be validated; it is not equal to the validation file {tempFilePathInfo.Length}." );
        }
    }
}
