...
public class ScriptEntityBaseCollection<TItem>( NodeEntityBase node ) : System.Collections.ObjectModel.KeyedCollection<string, TItem> where TItem : ScriptEntityBase
{
    ...
    public bool DeleteScript( Pith.SessionBase? session, string? scriptName )
    {
        ...
        if ( !session.IsNil( scriptName ) )
        {
            ...
            if ( FirmwareScriptBase.ScriptNameExists( this.EmbeddedScriptNames, scriptName ) )
                isNull = session.DeleteEmbeddedScript( scriptName );
            else
            {
    ...
    public bool IsEmbedRequired( Pith.SessionBase session, string scriptName, bool saveToNonVolatileMemory, bool convertToByteCode, bool isBootScript )
    {
    ...
    /// <summary>
    /// checks if save is required for the specified script. Presumes list of embedded scripts was
    /// retrieved.
    /// </summary>
    /// <remarks>   2024-09-09. </remarks>
    /// <param name="session">  The session. </param>
    ...
    {
        return this.IsEmbedRequired( session, script.Name, script.FirmwareScript.SaveToNonVolatileMemory, script.FirmwareScript.ConvertToByteCode, script.FirmwareScript.IsBootScript );
    }
