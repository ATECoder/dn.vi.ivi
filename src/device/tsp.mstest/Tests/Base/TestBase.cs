...
public abstract class TestBase
{
    ...
    public static void InitializeBaseTestClass( TestContext testContext )
    {
        ...
        try
        {
            ...
            _ = TestBase.Logger?.BeginScope( methodFullName );
        }
        catch ( Exception ex )
        {
            if ( TestBase.Logger is null )
                Console.WriteLine( $"Failed initializing the test class: {ex}", methodFullName );
            else
                TestBase.Logger.LogExceptionMultiLineMessage( "Failed initializing the test class:", ex );
