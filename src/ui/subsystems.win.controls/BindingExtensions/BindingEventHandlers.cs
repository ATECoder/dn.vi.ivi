...
namespace cc.isr.VI.SubsystemsWinControls.BindingExtensions;
...
public static class BindingEventHandlers
{
    ...
    internal static ConvertEventHandler DisplayX2EventHandler = ( sender, e ) => e.Value = _hexFormatProvider.Format( ( int ) (e.Value ?? 0) );
    ...
    internal static ConvertEventHandler DisplayRegisterEventHandler = ( sender, e ) => e.Value = _hexFormatProvider.Format( e?.Value ?? 0 );

    /// <summary> The parse status register event handler. </summary>
    internal static ConvertEventHandler ParseStatusRegisterEventHandler = ( sender, e ) => e.Value = _hexFormatProvider.ToServiceRequests( $"{e.Value}" );

    /// <summary> The parse standard register event handler. </summary>
    internal static ConvertEventHandler ParseStandardRegisterEventHandler = ( sender, e ) => e.Value = _hexFormatProvider.ToStandardEvents( $"{e.Value}" );
