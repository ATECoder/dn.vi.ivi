...
# if false
    ...
    public static string BuildSessionIdentityReport( Ivi.Visa.IVisaSession? visaSession )
    {
        ...
        foreach ( Ivi.Visa.ConflictManager.VisaImplementation visaLibrary in new Ivi.Visa.ConflictManager.ConflictManager().GetInstalledVisas( ApiType.DotNet ) )
        {
            try
            {
                string fileName = visaLibrary.Location.Substring( visaLibrary.Location.IndexOf( ',' ) + 1 );
                _ = sb.AppendLine( $"Loading assembly from '{fileName}'..." );
                ...
                {
                    _ = sb.AppendLine( $"*** Failed to load assembly from {fileName}." );
                    continue;
                }
                else
                {
                    Version? installedVersion = installedAssembly.GetName().Version;
                    _ = sb.AppendLine( $"\tLoaded {installedAssembly.FullName}." );
                    _ = sb.AppendLine( $"\tVersion: {System.Diagnostics.FileVersionInfo.GetVersionInfo( installedAssembly.Location ).FileVersion}." );
            ...
            _ = sb.AppendLine( $"\tUnloading assembly." );
        }
        return sb.ToString();
    }
