    ...
    public static Assembly? TryFindImplementation( out string details )
    {
        ...
        foreach ( Ivi.Visa.ConflictManager.VisaImplementation visaLibrary in new Ivi.Visa.ConflictManager.ConflictManager().GetInstalledVisas( Ivi.Visa.ConflictManager.ApiType.DotNet ) )
        {
            ...
            try
            {
                ...
                (loadedAssembly, line) = GacLoader.TryFindLoadedAssemblyByFriendlyName( fileName );
                ...
                else
                {
                    ...
                    line = $"\tFull Name: {loadedAssembly.GetName().FullName}.";
                    _ = sb.AppendLine( line );
                    line = $"\tLocation:  {fileInfo.DirectoryName}.";
                    _ = sb.AppendLine( line );
                    line = $"\tFile Name: {fileInfo.Name}{fileInfo.Extension}.";
                    _ = sb.AppendLine( line );
                    line = $"\tProduct:   {versionInfo.ProductVersion}.";
                    _ = sb.AppendLine( line );
                    line = $"\tFile:      {versionInfo.FileVersion}.";
                    _ = sb.AppendLine( line );
                }
            }
            catch ( Exception exception )
