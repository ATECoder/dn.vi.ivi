...
public class VersionInfo
{
    ...
    public VersionInfo() : base() => this.ClearThis();
    ...
    /// <param name="identity"> Specifies the instrument identity string, which includes at a minimum the
    ...
    public virtual void Parse( string identity )
    {
        ...
        else
        {
            ...
            Queue<string> idItems = new( identity.Split( ',' ) );
            ...
            (this.ModelFamily, this.ModelFamilyMask) = VersionInfo.ParseModelFamily( this.Model, VersionInfo.ModelFamilyMasks, VersionInfo.ModelFamilies );
        }
    }
    ...
    protected virtual void ParseFirmwareRevision( string revision )
    {
        ...
        else
        {
                ...
                foreach ( string v in values )
                {
                    ...
                    if ( !string.IsNullOrWhiteSpace( vv ) )
                    {
                        foreach ( char c in vv.ToUpperInvariant().ToCharArray() )
                        {
                            ...
                            if ( builder.Length > 0 )
                                _ = builder.Append( '.' );
                            _ = builder.Append( value.ToString( System.Globalization.CultureInfo.InvariantCulture ) );
            ...
            this.FirmwareVersion = rev ?? new Version();
        }
    }
    ...
    /// <summary>   Gets a value indicating whether this object is tsp model. </summary>
    /// <value> True if this object is tsp model, false if not. </value>
    public bool IsTspModel => this.ModelFamily.StartsWith( "26", StringComparison.OrdinalIgnoreCase )
        || this.ModelFamily.StartsWith( "37", StringComparison.OrdinalIgnoreCase );
